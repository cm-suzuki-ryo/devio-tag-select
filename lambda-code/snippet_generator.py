...
def generate_snippet_with_ai(text, model_id, title=""):
    ...
    prompt_parts = [
        ...
        "要件:",
        "- 2文構成で最大160文字以内",
        "- 第1文（80文字以内）: 最重要な価値・成果を明示",
        "- 第2文（80文字以内）: 具体的な方法・内容で補足",
        "- タイトルと重複しない内容で補完する", 
        "- 記事の結論・まとめを優先して反映",
        f"- 主要キーワードを含める: {keywords_text}",
        "- 検索ユーザーの行動を促す表現",
        ""
    ]
...
if __name__ == "__main__":
    ...
    test_event = {
        ...
        "content": "AWS Lambdaは、サーバーを管理することなくコードを実行できるコンピューティングサービスです。従来のサーバー管理の手間を省き、開発者はビジネスロジックに集中できます。この記事では、Lambda関数の作成から実際のデプロイまでの手順を詳しく解説します。",
