        ...
        return {
            ...
            'body': json.dumps({
                ...
                'processing_flow': {
                    ...
                    'step4': f'LLM ranking with {model_id} completed, top 20 selected'
                }
            }, ensure_ascii=False)
        }
...
def evaluate_tags_with_llm(text, tag_scores, model_id):
    ...
    # LLMプロンプト
    prompt = f"""以下の記事内容に対して、各タグの適合度を1-10で評価してください。
...
各タグについて、記事内容との関連性を1-10で評価し、以下の形式で出力してください：
タグID:スコア
...
"""
    ...
    # LLM評価結果をパース
    llm_scores = parse_llm_scores(result_text)
    ...
    for tag in tag_scores:
        ...
        final_score = tag['score'] + (llm_score * 20)
...
def parse_llm_scores(result_text):
    """LLMの評価結果をパース"""
    scores = {}
