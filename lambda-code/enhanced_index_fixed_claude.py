...
def lambda_handler(event, context):
    try:
        ...
        # 3. LLMで全100タグを評価（改良版）
        final_tags, ranking_cache_info = evaluate_all_tags_with_llm_fixed(
        ...
        return {
            ...
            'body': json.dumps({
                ...
                'processing_flow': {
                    ...
                    'step4': f'LLM 100-scale evaluation of all {len(tag_scores)} tags completed (fixed parser), top 20 selected'
                }
            }, ensure_ascii=False)
        }
...
def parse_llm_scores_improved(result_text):
    """LLMの評価結果をパース（改良版）"""
    import re
