...
try:
    ...
    try:
        ...
        try:
            ...
            MECAB_AVAILABLE = True
            print("MeCab initialized with explicit dictionary path")
        except:
...
def enhanced_pre_filter_tags_100(blog_text, all_tags, max_tags=100):
    ...
    print(f"Extracted keywords: {keywords[:20]}...")  # デバッグ用
...
def evaluate_all_tags_with_llm_final(text, tag_scores, model_id):
    ...
    print(f"\n=== PARSED SCORES ===")
    print(f"Parsed {len(llm_scores)} scores: {dict(list(llm_scores.items())[:5])}...")
...
def lambda_handler(event, context):
    try:
        ...
        print(f"Article length: {len(blog_text)} characters")
        
        # タグデータ取得
        tags_data, tags_hash = get_tags_from_contentful_cached()
        print(f"Total tags available: {len(tags_data)}")
        ...
        # 1. 長文記事の場合は要約作成
        if is_long_article:
            print("Creating summary for long article...")
        ...
        print("Filtering tags with MeCab processing...")
        ...
        print("Evaluating tags with Claude LLM...")
